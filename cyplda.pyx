...
cpdef void init_topics(int[:, ::1] documents, double[:, ::1] K_V, double[:, ::1] D_K, int[::1] sum_K, int[::1] curr_K) nogil:
    cdef size_t i,j,k
    # number of topics
...
'''
Single iteration of CGS with dynamic sampling
- document and word start/step parameters to perform partial CGS
'''
@cython.boundscheck(False)
cpdef void CGS_iter(int[:, ::1] documents, double[:, ::1] K_V, double[:, ::1] D_K, int[::1] sum_K, int[::1] curr_K, double alpha, double beta, int[:, :, ::1] sampling, double[::1] p_K, int[::1] uniq_K, int thread_num, int num_threads) nogil:
    cdef size_t i,j,k,p
    # number of documents
    ...
    for i in xrange(D):
        for j in xrange(V):
            ...
            if documents[i,j] > 0:
                #check if document belongs to thread
                if thread_num <= i and (i-thread_num)%num_threads == 0:
